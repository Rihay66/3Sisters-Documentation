...
For example: [[Quad Renderer]] requires a loaded [[Shader]] and optionally a set universal sprite size for any rendered quad sprite (also indirectly the [[Shader]]'s projectionView must be set utilizing the [[Camera]] module)
``` cpp
...
Then the [[Quad Renderer]] is now able to render (Also note that to render you must use the [[Window]] function paradigm if you are utilizing the engine's [[Window]] module)
```cpp
#include <engine/sprite_renderer.hpp>
...
[[Quad Renderer]] - for handling quad, line, rectangle rendering
